...
import numpy as np
import proximal_operator as prxopt


def SolveFISTA( xk, A, b, ind, gamma, glms, method ):
    ...
    for k in xrange(0,ntasks):
        xt_y[k*ndim:(k+1)*ndim] = np.absolute( np.dot(A.T,b[k]) )
    ...
    lambda2_max = prxopt.compute_largest_group_norm( larg_l1, ind, ndim, ntasks )
...
def cost_function( w, x, y, ind, glms, alpha1, alpha2 ):
    ...
    for t in xrange(0,ntasks): # group lasso is applied to all tasks independently
